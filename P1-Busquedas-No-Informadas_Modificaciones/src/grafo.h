...
struct SearchResult {
  ...
  int nodo_seleccionado{}; // AÑADIDO
  int numero_intentos{}; // AÑADIDO
};
...
class Grafo {
 ...
 private:
  ...
  std::vector<int> ReconstruirCamino(const std::unordered_map<int, int>&, int, int) const;
  double CalcularCostoCamino(const std::vector<int>&) const;
